...
theorem neg_eq_of_add_eq_zero {a b : R} (h : a + b = 0) : -a = b := by
  rw [← add_neg_cancel_right b a, add_comm b, h, zero_add]
...
theorem mul_inv_cancel (a : G) : a * a⁻¹ = 1 := by
  ...
  rw [← mul_assoc, mul_assoc (a⁻¹⁻¹), inv_mul_cancel, mul_assoc, one_mul]

theorem mul_one (a : G) : a * 1 = a := by
  rw [← inv_mul_cancel a, ← mul_assoc, mul_inv_cancel, one_mul]

theorem mul_inv_rev (a b : G) : (a * b)⁻¹ = b⁻¹ * a⁻¹ := by
  rw [← mul_one (b⁻¹ * a⁻¹), ← mul_inv_cancel (a * b)]
  rw [← mul_assoc, ← mul_assoc, mul_assoc (b⁻¹)]
  rw [inv_mul_cancel, mul_one, inv_mul_cancel, one_mul]
